...
//        descriptorWrites[1].pImageInfo = &imageInfo;
    ...
    void shaderFillDescriptorSets(const ShaderName &shader,
        ...
        switch (shader) {
            case grass: {
                fillDescriptor.ubo = true;
                fillDescriptor.image = true;
                break;
            }
            case selectionBox: {
                fillDescriptor.ubo = true;
        ...
        if (fillDescriptor.image) {
            fillDescriptor.imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            fillDescriptor.imageInfo.imageView = textureImageView;
            fillDescriptor.imageInfo.sampler = Texture::m_textureSampler;
        }
        for (auto i = 0; i < size; i++) {
            ...
            int a = 0;
            if (fillDescriptor.ubo) a++;
            if (fillDescriptor.image) a++;
            std::vector<VkWriteDescriptorSet> descriptorWrites{static_cast<size_t>(a)};

            if (fillDescriptor.ubo) {
                descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
                descriptorWrites[0].dstSet = descriptorSets[i];
                descriptorWrites[0].dstBinding = 0;
                descriptorWrites[0].dstArrayElement = 0;
            ...
            if (fillDescriptor.image) {
                ...
                descriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                descriptorWrites[1].descriptorCount = 1;
                descriptorWrites[1].pImageInfo = &fillDescriptor.imageInfo;
            }
            vkUpdateDescriptorSets(Gra::m_device,
                                   static_cast<uint32_t>(descriptorWrites.size()),
                                   descriptorWrites.data(),
