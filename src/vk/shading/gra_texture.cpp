...
VkSampler
    m_textureSampler; // Holds information about how to render like e.g. filters
std::map<std::string, VkImageView> texImageViews{};
...
void createImage(uint32_t width, uint32_t height, VkFormat format,
  ...
  allocInfo.memoryTypeIndex =
      Gra::findMemoryType(memRequirements.memoryTypeBits, properties);

  if (vkAllocateMemory(Gra::m_device, &allocInfo, nullptr, &imageMemory) !=
      VK_SUCCESS) {
...
void createTextureImage(ImageData &texture, TextureData *texData) {
  ...
  createImage(texture.w, texture.h, VK_FORMAT_R8G8B8A8_SRGB,
              VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, texData->textureImage,
              texData->textureImageMemory);

  // The next step is to copy the staging buffer to the texture image. This
  // involves two steps:

  transitionImageLayout(texData->textureImage, VK_FORMAT_R8G8B8A8_SRGB,
                        VK_IMAGE_LAYOUT_UNDEFINED,
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

  copyBufferToImage(stagingBuffer, texData->textureImage,
                    static_cast<uint32_t>(texture.w),
                    static_cast<uint32_t>(texture.h));

  // To be able to start sampling from the texture image in the shader, we need
...
VkImageView createTexture(ImageData &img) { // TODO Maybe save imgs for later in
                                            // case they're used more than once
  TextureData data{};
  createTextureImage(img, &data);
  data.textureImageView = Gra::createImageView(
      data.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
...
void createTextureSampler() {
  ...
  VkSamplerCreateInfo samplerInfo{
      ...
      // true then texels will first be compared to a value, and the result of
      // that comparison is used in filtering operations
      ...
      // specifies which coordinate system you want to use to address texels in
      // an image. True is 0->texHeight/Width, false is 0->1
      .unnormalizedCoordinates = VK_FALSE,
  };

  if (vkCreateSampler(Gra::m_device, &samplerInfo, nullptr,
...
std::vector<VkImageView> getTexImageViews(std::vector<std::string> textures) {
  ...
  for (auto &texture : textures) {
    ...
    res.emplace_back(texImageViews[texture]);
  }
  return res;
}
